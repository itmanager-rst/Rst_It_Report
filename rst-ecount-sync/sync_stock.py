...
def handle_get_stock():
    """ดึงข้อมูล สต็อก + Min/Max จาก BigQuery"""
        ...
        try:
            query = f"""
                ...
                SELECT 
                    ...
                    s.stock_qty AS QTY, 
                    -- 📌 แปลง Timezone ให้เป็นเวลาประเทศไทย (Asia/Bangkok) ก่อนส่งกลับไปหน้าเว็บ
                    COALESCE(
...
def handle_save_minmax_item():
    """รับค่าการอัปเดต Min/Max สินค้าแบบรายชิ้น ใช้คำสั่ง MERGE เพื่อสร้าง/แก้ไขใน BigQuery"""
    ...
    max_qty = req_data.get("MAX_QTY", req_data.get("max_qty", 0))
    ...
    # 2. บันทึกลง BigQuery ด้วยคำสั่ง MERGE (Upsert)
    bq_client = get_bq_client()
    if bq_client:
        try:
...
def sync_current_stock():
    ...
    # 📌 บันทึก String เวลาแบบ ISO 8601 UTC
    ...
    for item in raw_stock_list:
        ...
        # ใช้ชื่อมาตรฐานตามรหัสคลังเสมอ เพื่อไม่ให้ชื่อสะกดต่างกัน
        # เช่น "เบญจลักษ์" กับ "เบญจลักษณ์" ทำให้หน้าเว็บกรองไม่เจอ
...
if __name__ == "__main__":
    # 🔄 1. รันดึงข้อมูลและอัปเดต timestamp ทันที 1 รอบตอนเปิดเซิร์ฟเวอร์
    ...
    # ⏰ 2. ตั้งเวลา Auto-Sync ทุกๆ 15 นาทีสำหรับรอบถัดไป
